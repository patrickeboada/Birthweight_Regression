...
CleanGGdad_age <- ggplot(data=births, mapping=aes(x=births$dad_age)) + geom_histogram(color = 'azure4', fill = 'lightblue', bins = 45) + ggtitle("Histogram for dad_age") + labs(x='Years', y='Frequency') + theme_bw() + theme(
    plot.title = element_text(hjust = 0.5),
    text=element_text(size=10,  family="Orbitron Light", face ='plain'),
    plot.background = element_blank(),
    panel.grid.major = element_blank(),
...
drop bwt and is_premie for as strongly correlated response variable. Also drop mom_age_level as it is derived from mom_age
